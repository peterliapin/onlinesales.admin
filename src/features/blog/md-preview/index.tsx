...
export const MdPreview = ({ source }: MdPreviewProps) => {
  ...
  const customLinkMatches = findMatches(/\s*:a\[.*\]\{(?:[^{}]|())*\}/gi, filteredSource);
  if (customLinkMatches) {
    for (let i = 0; i < customLinkMatches.length; i++) {
      ...
      const paramsMatch = /\{(?:[^{}]|())*\}/gi.exec(customLinkMatch);
      filteredSource = filteredSource.replace(
        customLinkMatch,
        `<customLink text="${text}" ${paramsMatch && paramsMatch[0]} />`
      );
    }
  }

  const customImageMatches = findMatches(/\s*!\[.*\](.*?)\(.*\)/gi, filteredSource);
  ...
  if (customImageMatches) {
    for (let i = 0; i < customImageMatches.length; i++) {
      ...
      filteredSource = filteredSource.replace(
        customImageMatch,
        `<customImg src="${(coreApi || "") + url}" alt="${alt || ""}" />`
      );
    }
  }
